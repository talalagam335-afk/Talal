...
// --- Stages 3-4 output: match analysis + CV ---------------------------------
// The generation stage produced on Day 4: the Match Analysis (strategy) plus
// the tailored CV, both constrained to the Source of Truth. The cover letter
// (Day 5) is generated by a separate stage and merged in the orchestrator.

export const AnalysisAndCvSchema = z.object({
  matchAnalysis: MatchAnalysisSchema,
  cv: z.string(),
});
export type AnalysisAndCv = z.infer<typeof AnalysisAndCvSchema>;

/** Safe-parse an unknown value (e.g. AI output) as an AnalysisAndCv. */
export function parseAnalysisAndCv(data: unknown) {
